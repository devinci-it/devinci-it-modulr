...
namespace DevinciIT\Modulr\Components\Forms\Radio;

use DevinciIT\Modulr\Components\ComponentsBase;

class Radio extends ComponentsBase
{
    ...
    public function setSize(string $size): static
    {
        ...
        if (!in_array($size, $this->allowedSizes, true)) {
            throw new \InvalidArgumentException(sprintf('Invalid radio size: %s', $size));
        }
    ...
    public function render(): string
    {
        $id = $this->props['id'];
        if (!$id) {
            $id = 'modulr-radio-' . substr(md5(uniqid((string) mt_rand(), true)), 0, 8);
        }
        ...
        $attributes = $this->mergeBaseAttributes([
            'class' => 'modulr-radio__input modulr-radio__input--' . $this->props['size'],
            'id' => $id,
